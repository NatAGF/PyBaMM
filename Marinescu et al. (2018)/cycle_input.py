...
def cycle_func(I):
    ...
    for i in I:
        ...
        if I < 0:  # on charge, allow shuttle
            k_s0 = 0.00003  # at reference temperature T0
        else:
        ...
        mc = 50  # cell mass in g TODO: should be a function of m_s
        ch = 2  # J/(g K). cell specific heat capacity
        h = 0.02  # W/k. total cell heat transfer coefficient  TODO: should be a function of mc or m_s
        ...
        if counter >= 1:  # TODO: code doesn't always run if initial conditions are defined this way. Maybe we need to rest it?
            print(S8_initial, S4_initial, Sp_initial)
            S8_initial=S8_sol[-1]
            S4_initial = S4_sol[-1]
            Sp_initial=Sp_sol[-1]
            Ss_initial = Ss_sol[-1]
        ...
        if I < 0:
            ...
            seconds = abs(CellQ / I * 3600) * 2  # allow for longer charge times due to shuttle during slow discharge
        ...
        dae_solver = pybamm.ScikitsDaeSolver(atol=1e-14, rtol=1e-8)  # TODO: decide on solver or test if both can be used
        ...
        t_sol_temp = solution.t
        for j in range(len(t_sol_temp)): # TODO: should be optimised to run faster, Michael knows how

            t_sol.append(t_sol_temp[j] + t_end)


        S8_sol = np.append(S8_sol,solution["S8"].data)
        S4_sol = np.append(S4_sol,solution["S4"].data)
        S2_sol = np.append(S2_sol,solution["S2"].data)
        S_sol = np.append(S_sol,solution["S"].data)
        Sp_sol = np.append(Sp_sol,solution["Sp"].data)
        Ss_sol = np.append(Ss_sol,solution["Ss"].data)
        V_sol = np.append(V_sol,solution["V"].data)
        T_sol = np.append(T_sol,solution["T"].data)
        ...
        t_end=t_sol[-1]
    return [t_sol, S8_sol, S4_sol, S2_sol, S_sol, Sp_sol, Ss_sol, V_sol, T_sol,E_H_sol,E_L_sol,eta_H_sol,eta_L_sol,i_H_sol,i_L_sol]
